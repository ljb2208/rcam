    ...
    )
{
    ...
    fprintf(stderr, "MGMT_RTSP: New RTSP server thread. [id = %d]", id);

    fprintf(stderr, "MGMT_RTSP: Client IP: %s", f_cblk.session[id].client_ip_str);
    ...
    while(1)
    {
        ...
        if(rv < 1)
        {
        	fprintf(stderr, "MGMT_RTSP: Client terminated TCP connection. [client IP: %s]",
                    f_cblk.session[id].client_ip_str);
            break;
        }

        // Log request.
        fprintf(stderr, "MGMT_RTSP: RTSP Request [session ID = %d]:", id);
        fprintf(stderr, "%s", msg_rx);
        ...
        fprintf(stderr, "MGMT_RTSP: RTSP Response [session ID = %d]:", id);
        fprintf(stderr, "%s", msg_tx);
    ...
    )
{
    ...
    while(1)
    {
        ...
        fprintf(stderr, "MGMT_RTSP: Received new TCP connection, initiating new RTSP sever instance...");
