    ...
    )
{
    ...
    fprintf(stderr, "Peer IP: 0x%x", session->peer_addr);
    ...
    )
{
    ...
    fprintf(stderr, "MGMT_RTP: ACTIVATE received [id = %d]",
...
{
    fprintf(stderr, "MGMT_RTP: RESET received [id = %d]",
    ...
    for(i = 0; i < 32; i++)
    {
        ...
        if(session->in_use)
        {
            while(1)
            {
                if(!session->sps_sent)
                {
                    fprintf(stderr, "Get SPS unit");

                    // Send SPS second.
                    sps_nal_unit = sx_mgmt_video_sps_get();
                    if(sps_nal_unit == NULL)
                    {
                        fprintf(stderr, "mgmt_video_get_sps_nal_unit() returned NULL!");
                        break;
                    }

                    fprintf(stderr, "SPS unit len %d", sps_nal_unit->nal_unit_len);
                ...
                {
                    fprintf(stderr, "Get PPS unit");

                    // Send PPS first.
                    pps_nal_unit = sx_mgmt_video_pps_get();
                    if(pps_nal_unit == NULL)
                    {
                        fprintf(stderr, "mgmt_video_get_pps_nal_unit() returned NULL!");
                        break;
                    }
